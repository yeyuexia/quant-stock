...
{ "ticker": str, "sector": str|None,
  ...
  "price_action":{price, pct_from_52w_high, pct_from_52w_low,
                  ...
                  atr14, swing_low_20, swing_high_20},
...
**`suggested_levels`** computes — deterministically, in code (never LLM-invented)
— an entry/exit plan from the dossier's price + `atr14` + structure:
`buy_low = price − buy_band_atr·atr14`, `buy_high = price + buy_band_atr·atr14`;
`stop_loss = min(swing_low_20, buy_low − stop_atr_mult·atr14)` (structure or
volatility, whichever is lower/safer); `take_profit = buy_high + target_r·(buy_high − stop_loss)`
(R-multiple). Returns `{buy_low, buy_high, stop_loss, take_profit}`, each rounded
to cents; any field is `None` when `price`/`atr14` is missing (fail-open). These
are **advisory** levels attached to each pick — the watchdog's own coded buy/exit
logic is unchanged.
...
7. **Attach levels + persist + monitor.** For each pick, compute
   `dossier.suggested_levels(...)` and attach `{buy_low, buy_high, stop_loss,
   take_profit}` (advisory). Write `buy_candidates.json` picks, enriched
   additively:
   `{ticker, rationale, signal, confidence, thesis, risks, catalysts,
   buy_low, buy_high, stop_loss, take_profit, strategies}`.
   Append one row to `.cache/agent_source_mix.csv`
   (`date, n_picks, n_value, n_canslim, n_other, shortlist_value, shortlist_canslim`)
   so source bias is **measurable over time**, and append a human-readable
   **reason summary** per pick to `.cache/agent_reasons.log`
   (`<date> <ticker> <signal> conf=<n> buy=<low>-<high> stop=<sl> tp=<tp> | <thesis>; risks: <risks>`).
...
- **dossier (pure, the core):** `build_dossier` fields from injected
  ...
  **`suggested_levels`** — buy band = price ± band·ATR, stop = min(swing_low,
  buy_low − mult·ATR), TP = R-multiple of risk; `None` levels when price/atr14 missing.
...
- **monitoring:** a run writes one `.cache/agent_source_mix.csv` row with correct
  value/canslim counts for shortlist and picks, and one `.cache/agent_reasons.log`
  line per pick containing its buy/stop/tp levels + thesis.
- **end-to-end:** injected fetchers + a stage-aware fake `llm_fn` → enriched
  `buy_candidates.json` with `ticker` (back-compat) + analysis fields + the four
  level fields (`buy_low, buy_high, stop_loss, take_profit`); owned excluded;
  all-LLM-fail → rule-rank balanced top-N (levels still attached).
...
1. config knobs + `quant/agent/dossier.py` (build_dossier + helpers + add_peer_relative + suggested_levels + compact_line) + tests.
...
3. Stage B analyst + Stage C critic + Stage D PM prompts/parsers + per-stage fallbacks + abstention + level attachment + enriched picks + tests.
4. source-mix monitoring + reasons-log + docs + dry-run.
```
