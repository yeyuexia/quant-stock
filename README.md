...
> 📊 **New here? Read the visual overview first:** [`docs/system_overview.html`](docs/system_overview.html) — a rich, self-contained page explaining how the whole system works at both overview and detail levels. For an **interactive** map (pan / zoom / drag / click nodes for details) see [`docs/architecture.html`](docs/architecture.html). Open either in a browser. Keep both updated whenever the system changes.
...
| `quant/news/` (`news_store` / `political` / `forecast`) + `quant/app/news_poller.py` + `quant/infra/tg_notifier.py` | **Political-forecast subsystem.** RSS ingestion + rule-based political-event classification → SQLite news store → LLM hotspot/briefing analysis. Feeds `quant/signals/macro.py::macro_risk_adjustment` (blends a political-risk score at 30% with the FRED macro score at 70%, FRED-only fallback) and the watchdog's `check_political_forecast` alert. `news_poller` is the polling daemon (`python3 -m quant.app.news_poller`); `tg_notifier` pushes Telegram briefings. The Telegram bot's `/forecast` + `/hotspots` read from it. |
...
- **`ENFORCE_STOPS`** (default `True`) — the intraday watchdog submits a **market sell** on a stop-loss or trailing-stop breach. Market orders work on fractional shares, where native stop/trailing-stop orders are rejected by Alpaca. The order is idempotent per `(symbol, day)`; the daily pre-market run stays alert-only (enforcement requires the live broker passed in the intraday tick). The trailing-stop peak is anchored to each position's `entry_date` (stamped at adoption/first-sight by `sync_state`), so a freshly-adopted holding is measured from when we began managing it — not against a pre-adoption high — and a stop-enforce sell also prunes any SEPA exit queued for that symbol so the position is never double-sold.
